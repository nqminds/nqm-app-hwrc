module.exports = (function(config, tdxAPI) {
  ...
  var nextTonnage = function(accessToken, aCost, max, index, delta, tonnageDS, outputDS, cb, retrying) {
  ...
  var uploadCostOutputData = function(accessToken, req, res){
    ...
    if(costCalc.working == true){
      ...
      tdxAPI.truncateDataset(accessToken, req.session.outputDS, function(err) {
        ...
        tdxAPI.query(accessToken, "datasets/" + req.session.costDS +  "/data", null, null, {"limit":10000}, function(err, qres, costData) {
          if(err){
          ...
          tdxAPI.query(accessToken, "datasets/" + req.session.tonnageDS +  "/count", null, null, null, function(err, qres, tonnageCount) {
            ...
            nextTonnage(accessToken, aCost, max, 0, 10000, req.session.tonnageDS, req.session.outputDS, function(nextRes){
              console.log(nextRes);
            })
          });
        });
