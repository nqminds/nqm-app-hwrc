module.exports = (function(config, tdxAPI) {
  ...
  var nextTonnage = function(accessToken, aCost, max, index, delta, tonnageDS, outputDS, cb) {
    ...
    if (index < max) {
      ...
      costCalc.progressString = String(("Calculated " + Math.round(100 * index / max) + "%: " + index.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") + " out of " + max.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") + " rows"));
      ...
      tdxAPI.query(accessToken, "datasets/" + tonnageDS + "/data", null, null, {
        "skip": index,
        "limit": delta
  ...
  var startCostOutputCalcs = function(accessToken, costDS, tonnageDS, outputDS, index_start, cb){
    ...
    tdxAPI.query(accessToken, "datasets/" + costDS +  "/data", null, null, {"limit":10000}, function(err, qres, costData) {
      ...
      tdxAPI.query(accessToken, "datasets/" + tonnageDS +  "/count", null, null, null, function(err, qres, tonnageCount) {
        ...
        nextTonnage(accessToken, aCost, max, index_start, 10000, tonnageDS, outputDS, function(err, nextRes){
  ...
  var uploadCostOutputData = function(accessToken, req, res){
    ...
    if(costCalc.working == true){
      ...
      tdxAPI.truncateDataset(accessToken, req.session.outputDS, function(err) {
        ...
        startCostOutputCalcs(accessToken, req.session.costDS, req.session.tonnageDS, req.session.outputDS, 0, function(err){
          if(err){
            console.log(err)
            costCalc.err = true;
          }
  ...
  var finishCostCalc = function(req, res){
    ...
    if(costCalc.working == true){
      ...
      getStartIndex(req, res, 0, function(index_start){
        ...
        startCostOutputCalcs(accessToken, req.session.costDS, req.session.tonnageDS, req.session.outputDS, index_start, function(err){
          if(err){
            console.log(err)
            costCalc.err = true;
          }
