module.exports = (function (RCLookupTable) {
  ...
  var buildURL = function(resourceId, pipeline, nidMode) {
    return util.format("/model/%s/?pipeline=%j&nidMode=%s", resourceId, pipeline, nidMode);
  };
    ...
    try {
      ...
      request.get(aggregateQuery, { json: true }, function (err, resp, body) {
        ...
        if (err || (resp && resp.statusCode !== 200)) {
          ...
          cb(null, { columns: columns, data: data, nidModeToggle: nidModeToggle, hwrcLookup: RCLookupTable });           
        }
      });        
    } catch (e) {
