module.exports = (function (RCLookupTable) {
  ...
  var aggregateTableFactory = function (accessToken, resourceId, pipelineParam, nidMode, cb) {
    ...
    var mainPipelineParam = '[{"$match":{"SID":"2021"}},{"$group":{"_id":{"NID":"$NID"},"Cost":{"$sum":"$Cost"}}}]'

    // if pipeline empty add the default pipeline
    if(pipelineParam == "[]"){
      pipelineParam = mainPipelineParam;
    }
    ...
    try {
      ...
      //if this is a mainPipeline Query check if it has been cached
      if (pipelineParam == mainPipelineParam && aggregateMainQueryList.hasOwnProperty(aggregateQuery)) {
          log("using cached main query")
          return cb(null, { columns: aggregateMainQueryList[aggregateQuery].columns, data: aggregateMainQueryList[aggregateQuery].data, hwrcLookup: RCLookupTable });
      }
      ...
      request.get(aggregateQuery, { json: true }, function (err, resp, body) {
        ...
        if (err || (resp && resp.statusCode !== 200)) {
          ...
          aggregateMainQueryList[aggregateQuery] = {};
          aggregateMainQueryList[aggregateQuery].columns = columns;
          aggregateMainQueryList[aggregateQuery].data = data;
