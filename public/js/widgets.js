/**
 * Created by leovalberg on 02/06/2016.
 */
...
WasteMap = function(widgetId, container, input_data){
    ...
    //self.boundaryUrl =oURL.boundaryUrl;

    self.component = new d3Components();
    ...
    var check_list = [
        {variable: "boundaryData", check: false},
        //{variable: "districtData", check: false},
        {variable: "poiData", check: false},
        {variable: "hwrcLookup", check: false}
    ];

    self.checkList = new CheckList(check_list, self._draw_map.bind(self));


    self._get_data(input_data.boundaryUrl, function(res){
        self.checkList._check("boundaryData", res)
    });
    ...
    self._get_data(input_data.hwrcLookupUrl, function(res){
        self.checkList._check("hwrcLookup", res)
    });
...
WasteMap.prototype._draw_map = function(data){
...
WasteMap.prototype._init = function(){
    ...
    self.chart = self.component._build_chart(self);

    var polygon_bucket_count = 4;
    var poi_bucket_count = 5; //must be odd

    var fPolygonFill = function(d){

        //console.log(d.properties)

        //if(d.properties.LAD14CD == "E07000093"){
        //    console.log(d.properties)
        //}
    ...
    var fPoiFill = function(d){
        ...
        for(var i = 0; i < options.length; i++){
                ...
                if(options[i].value.charAt(Bit_Index) == "1"){
                    flagOn = true
                }
                if(options[i].value.charAt(Bit_Index) == "0"){
                    flagOff = true
                }
    ...
    self.map = self.component._map(self, {
        id:"LAD14CD",
        polygon_style: {
            "opacity": 1,
            "fill": fPolygonFill
...
WasteMap.prototype._update = function(polygon_data, polygon_ranks, poi_data, poi_max){
    ...
    for(var polygon_index in self.polygon){
        ...
        for(var polygon_data_index in polygon_data){
            if(polygon_data[polygon_data_index].id == id){
                ...
                //if(id == "E07000093"){
                //    console.log(polygon_data[polygon_data_index])
                //    console.log(self.polygon_ranks)
                //}
    ...
    for(var poi_index in self.poi){
        ...
        var bit_index = self._lookupHwrc("GeoJson_ID", inProp, "Bit_Index");
        for(var poi_data_index in poi_data){
            if(poi_data[poi_data_index].id == id){
                ...
                self.poi[poi_index].properties.bit_index = bit_index;
                break
            }
        }
...
WasteMap.prototype._get_data = function(input, callback){

    if(typeof(input.constructor) === Array){ //if it is the data array and not a url pass it straight back
        callback(input)
    } else {
        //todo check if url
        $.ajax(input).done(function(res){
            callback(res.data);
        });
    }
};
